...
- A run is **superseded** when a release tag with a higher run id exists (e.g. an old failed run re-run). It still publishes `<tools>-r<N>` and its release, but never moves `:<tools>`, `:latest` or the "latest" GitHub Release. It still creates or repairs `:sha-*` when the build matches that commit's own pins. This is evaluated inside `build`/`release`, not `version`: "re-run failed jobs" reuses outputs of jobs that succeeded
