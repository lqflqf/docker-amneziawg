...
3. **MTU.** `awg-quick` derives 1420 without accounting for `S4`, so full-size datagrams exceed 1500 when `S4 > 20` (IPv4) or on any IPv6 endpoint, and fragment. Use `MTU = 1280` on mobile/PPPoE/unknown paths, `1500 − 60 − S4` on a clean IPv4 path — and `path − 60 − S4` (IPv4) / `path − 80 − S4` (IPv6) when the path is itself constrained: a true 1280-byte path needs 1208/1188, since 1280 there still fragments. Details: README "MTU" and [`docs/awg-performance.md`](../../../../docs/awg-performance.md).
