...
- CI smoke tests (`.github/scripts/smoke-test.sh`) also check that `svc-unbound/run`, `svc-amneziawg/finish` and `/app/healthcheck` are executable, that the seeded `unbound.conf` passes `unbound-checkconf` and the running Unbound process is `abc`, and that the healthcheck and s6 report a tunnel that did not come up as down
...
### Security
- Unbound drops privileges to `abc` after binding port 53 (`username: "abc"`). Existing `/config/unbound/unbound.conf` files that still carry the old shipped `username: ""` line are migrated on start; set `username: "root"` to opt out
